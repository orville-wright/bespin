...
class hedgeweek_news:
    """
    Class to crawl data from Hedgeweek via Crawl4ai
    ...
    async def craw4ai_str_schema_extr(self):
        ...
        # WARN: Each url must match the JSON schema in: HEDGEWEEK_crawl4ai_schema.json or the
        #       crawl4ai scraper will not extract any data.
        
        urls = [
            "https://www.hedgeweek.com/news"
        ]
        url = "https://www.hedgeweek.com/news"
        ...
        for dict in self.DF_data:                   # this is where the final dataset can be accessed
            ...
            if url.startswith('/'):
                url = 'https://www.hedgeweek.com' + url
        ...
        logging.info(f"{cmi_debug} - complete Hedgeweek data crawl/scrape" )
...
if __name__ == "__main__":
    ...
    async def test_hedgeweek_news():
        logging.info("Testing Hedgeweek News Extractor...")
        hedgeweek_extractor = hedgeweek_news(1)
        await hedgeweek_extractor.craw4ai_str_schema_extr()
        logging.info(f"Extracted {len(hedgeweek_extractor.DB_insert_data)} Hedgeweek news items")
        
    asyncio.run(test_hedgeweek_news())