...
class yfnews_reader:
    ...
    yfn_jsdb = {}           # database to hold response handles from multiple get() ops)
...
###################################### 7 ###########################################
# HTML page data reader: Javascrip engine
...
###################################### 7.1 ###########################################
# HTML page data reader: Playwright engine
...
###################################### 8 ###########################################
# HTML page data reader: Basic request_get()
...
###################################### 9 ###########################################
    def scan_news_feed(self, symbol, depth, scan_type, bs4_obj_idx, hash_state):
        """
        NO html get() network data read. Just process craw4ai data obejct scraped
        ...
        if scan_type == 1:  # crawl4ai extraction
            ...
            try:
                ...
                if isinstance(self.extracted_articles, list):       # test for list
                    ...
                    if self.args.get('bool_xray', False):           # Debug
                        print(f" ")
        ...
        # old BS4 code... structurally unreachable : Delete Me !!
        """
        Symbol : Stock symbol NEWS FEED for articles (e.g. https://finance.yahoo.com/quote/OTLY/news?p=OTLY )
        Depth 0 : Surface scan of all news articles in the news section for a stock ticker
...
###################################### 10 ###########################################
    def eval_news_feed_stories(self, symbol):
        """
        Depth 1 - scanning news feed stories @ stock home page. gather initial high-level metatdata
        INFO: NOT looking deeply inside each  article yet
...
###################################### 11 ###########################################
    def interpret_page(self, item_idx, data_row):
        """
        Depth 2 Page interpreter - currently uses BS4
