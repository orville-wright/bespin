...
# Must be given ml_ingest DB 
    def interpret_page(self, item_idx, data_row):
        ...
        # This is the Data Extrtactor engine
        # currently BS4
...
###################################### 12 ###########################################
    ...
    def extract_article_data(self, item_idx, sentiment_ai):
        """
        Depth 3:
        Extart all the doc/page html TEXT from this page, for AI NLP to parse it through NTLK sentiment analyhzer
        Only do this once the article has been evaluated and we know exactly where/what each article is
        Any article we read, should have its resp & BS4 objects cached in yfn_jsdb{}  - BS4 mode
        Set the Body Data zone, the <p> TAG zone
        Extract all of the full article raw text
        Store it in a Database
        Associate it to the meta data info for this article
        Its now available for the LLM (local RoBERTa model for now), to read and process
        """
