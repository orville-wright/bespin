...
class ml_nlpreader:
    ...
    async def async_nlp_read_one(self, news_symbol, global_args):
        ...
        """
        print(" ")
        print(f"AI NLP / REad News Sentiment for 1 symbol [ {news_symbol} ]")
        self.args = global_args
        ...
        # Create crawl4ai-based Yahoo Finance News reader
        ...
        # Use crawl4ai to extract news data
    ...
    def nlp_summary(self, yti, ml_idx):
        ...
        **CRITICAL: Assumes ml_ingest has already been pre-populated
        ...
        if sn_row['type'] == 0:  # REAL valid news article
            ...
            print(f"{sn_row['symbol']} / Video article - Not readable: {ml_idx} - AI will not Read sentimentt")
            t_url = urlparse(sn_row['url'])
            thint = sn_row['thint']
            inf_type = self.yfn_uh.confidence_lvl(thint)
            ...
            print(f"{sn_row['symbol']} / External Publiation - Not readable: {ml_idx} - AI will not Read sentiment")
            t_url = urlparse(sn_row['url'])
            thint = sn_row['thint']
            inf_type = self.yfn_uh.confidence_lvl(thint)
            print(f"Article: {ml_idx} - {inf_type[0]}: 2 - AI will not Read sentiment")
            ...
            thint = sn_row['thint']
            inf_type = self.yfn_uh.confidence_lvl(thint)
            print(f"Article: {ml_idx} - {inf_type[0]}: 5 - AI will not Read sentiment")
            logging.info(f"%s - skipping..." % cmi_debug)
            return thint
            
        elif sn_row['type'] == 9:  # Not yet defined
            print(f"Article: {ml_idx} - Type 9 - NOT yet defined - AI will not Read sentiment")
            logging.info(f"%s - skipping..." % cmi_debug)
            thint = sn_row['thint']
            return thint
            
        else:
            print(f"Article: {ml_idx} - ERROR unknown article type: {sn_row['type']}")
            logging.info(f"%s - #? skipping..." % cmi_debug)
            thint = sn_row['thint']
            return thint
    ...
    def process_sentiment_analysis(self, target_symbols=None):
        ...
        for item_idx, data_row in self.ml_yfn_dataset.ml_ingest.items():
            if data_row.get('viable', 0) == 1:  # Only process viable articles
                ...
                logging.info(f'%s - Processing sentiment for article {item_idx}: {symbol}' % cmi_debug)
