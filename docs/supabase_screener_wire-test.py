"""
Supabase wire test for screened_candidate_targets.

Proves the payloads produced by csv_smoke.py actually survive PostgREST:
insert, read back, null-beta round-trip, upsert idempotency, constraint
...
CONFLICT         = "symbol,screener_name,target_session"   # exact, no spaces
SENTINEL_SESSION = date(2099, 1, 1)                        # never a real session
...
def main() -> int:
    ...
    try:
        # -- pre-clean, in case an earlier run aborted -------------
        sb.table(TABLE).delete().eq("target_session", sess).execute()

        # -- W01 insert ------------------------------------------
        try:
            sb.table(TABLE).upsert(
                rows, on_conflict=CONFLICT, ignore_duplicates=False
            ).execute()
        ...
        # -- W02 read back ---------------------------------------
        resp = (sb.table(TABLE)
                  .select("symbol,metrics,screened_at,collector")
                  .eq("target_session", sess)
                  .order("symbol").execute())
        check("W02 row count after insert", 23, len(resp.data), len(resp.data) == 23)

        # -- W03 null beta survived ------------------------------
        agbk = next((r for r in resp.data if r["symbol"] == "AGBK"), None)
        got = agbk["metrics"]["beta"] if agbk else "MISSING"
        check("W03 AGBK beta is null after round-trip", None, got,
              agbk is not None and got is None)

        # -- W04 float precision preserved -----------------------
        move = next((r for r in resp.data if r["symbol"] == "MOVE"), None)
        ...
        # -- W06 timestamptz came back UTC -----------------------
        ts = move["screened_at"] if move else ""
        check("W06 screened_at UTC", "+00:00 or Z", ts[-6:],
              ts.endswith("+00:00") or ts.endswith("Z"))

        # -- W07 upsert is idempotent ----------------------------
        sb.table(TABLE).upsert(
            rows, on_conflict=CONFLICT, ignore_duplicates=False
        ).execute()
        again = (sb.table(TABLE).select("id", count="exact")
                   .eq("target_session", sess).execute())
        check("W07 re-run does not duplicate", 23, again.count, again.count == 23)

        # -- W08 second screener, same symbols -------------------
        rows_b = build_rows(SENTINEL_SESSION, "wire_test_b", "wilbur-akl")
        sb.table(TABLE).upsert(
            rows_b, on_conflict=CONFLICT, ignore_duplicates=False
        ).execute()
        both = (sb.table(TABLE).select("id", count="exact")
                  .eq("target_session", sess).execute())
        check("W08 two screeners coexist", 46, both.count, both.count == 46)

        # -- W09 constraints reject at the API layer -------------
        bad_cases = [
        ...
        for label, override in bad_cases:
            ...
            try:
                ...
                rejected.append(f"{label}=rejected")
        all_rejected = all(r.endswith("rejected") for r in rejected)
        check("W09 bad rows rejected by API", "all rejected",
              "all rejected" if all_rejected else "; ".join(rejected), all_rejected)

        # -- W10 view is readable --------------------------------
        try:
            view = (sb.table("v_daily_conviction").select("*")
                      .eq("target_session", sess).execute())
            aapl_like = next((v for v in view.data if v["symbol"] == "MOVE"), None)
            ok = (aapl_like is not None
                  and aapl_like["screener_count"] == 2
                  and aapl_like["cross_confirmed"] is True)
            check("W10 view: MOVE cross-confirmed", "2 screeners, true",
                  f"{aapl_like['screener_count']} screeners, "
                  f"{aapl_like['cross_confirmed']}" if aapl_like else "not found", ok)
        except Exception as e:
            check("W10 view: MOVE cross-confirmed", "2 screeners, true",
                  f"ERROR: {e}", False)
        ...
        try:
            sb.table(TABLE).delete().eq("target_session", sess).execute()
            left = (sb.table(TABLE).select("id", count="exact")
                      .eq("target_session", sess).execute())
            check("W11 cleanup", 0, left.count, left.count == 0)
        except Exception as e:
