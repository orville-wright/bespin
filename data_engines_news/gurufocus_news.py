...
# ###################### Main class
class gurufocus_news:
    """
    Class to crawl data from GuruFocus via Crawl4ai
    Using CSS Structured JSON Schema method
    Schema is in JSON file: /json/GURUFOCUS_crawl4ai_schema.json
    """
    ...
    def __init__(self, inst_id):
        ...
        self.json_file = f"{self.cur_dir}/json/GURUFOCUS_crawl4ai_schema.json"
    ...
    async def craw4ai_str_schema_extr(self):
        ...
        # WARN: Each url must match the JSON schema in: GURUFOCUS_crawl4ai_schema.json or the
        #       crawl4ai scraper will not extract any data.
        
        urls = [
            "https://www.gurufocus.com/news?page=1&per_page=20&cat=timeless"
        ]
        url = "https://www.gurufocus.com/news?page=1&per_page=20&cat=timeless"
        ...
        for dict in self.DF_data:                   # this is where the final dataset can be accessed
            ...
            if url.startswith('/'):
                url = 'https://www.gurufocus.com' + url
        ...
        logging.info(f"{cmi_debug} - complete GuruFocus data crawl/scrape" )
...
if __name__ == "__main__":
    ...
    async def test_gurufocus_news():
        logging.info("Testing GuruFocus News Extractor...")
        gurufocus_extractor = gurufocus_news(1)
        await gurufocus_extractor.craw4ai_str_schema_extr()
        logging.info(f"Extracted {len(gurufocus_extractor.DB_insert_data)} GuruFocus news items")
        
    asyncio.run(test_gurufocus_news())